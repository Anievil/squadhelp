...
export function* getAllUsers(action){
    try{
        const {data}=yield restController.getAllUsers(action.data);
        yield put({type: ACTION.GET_ALL_USERS_SUCCESS, data});
    }
    catch (e) {
        yield  put({type: ACTION.GET_ALL_USERS_ERROR, error: e.response});
    }
}
...
export function* recoverySaga(action){
    try{
        const {data}=yield restController.recoverysRequest(action.data);
        yield put({type: ACTION.CHANGE_PASS_SUCCESS});
    }
    catch (e) {
        yield  put({type: ACTION.CHANGE_PASS_ERROR, error: e.response});
    }
}

export function* updatePassSaga(action){
    try{
        const {data}=yield restController.updatePassRequest(action.data);
        yield put({type: ACTION.CHANGE_PASS_SUCCESS});
    }
    catch (e) {
        yield  put({type: ACTION.CHANGE_PASS_ERROR, error: e.response});
...
export  function* privateSaga(action){
    ...
    try{
        const {data}=yield  restController.getUser(action.data);
