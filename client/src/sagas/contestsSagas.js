...
export function* getContests(action){
    try{
        const {data} = yield restController.getContests(action.data)
        yield put({type: ACTION.GET_CONTESTS_SUCCESS, data})
    }catch(e){
        yield put({type: ACTION.GET_CONTESTS_ERROR, error: e.response})
    }
}

export function* addNewContest(action){
    try{
        const {data} = yield restController.addNewContest(action.data)
        yield put({type: ACTION.ADD_NEW_CONTEST_SUCCESS, data})
    }catch(e){
        yield put({type: ACTION.ADD_NEW_CONTEST_ERROR, error: e.response})
    }
}
