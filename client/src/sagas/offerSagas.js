...
export function* acceptOffer(action){
    try{
        const {data} = yield restController.acceptOffer(action.data)
        yield put({type: ACTION.ACCEPT_OFFER_SUCCESS, data})
    }catch(e){
        yield put({type: ACTION.ACCEPT_OFFER_ERROR, error: e.response})
    }
}
...
export function* addNewOfferSaga(action){
    try{
        const {data} = yield  restController.addNewOffer(action.data);
        yield put({type: ACTION.ADD_NEW_OFFER_SUCCESS, data})
    }catch(err){
        yield put({type: ACTION.ADD_NEW_OFFER_ERROR, error: err.response})
    }
}
