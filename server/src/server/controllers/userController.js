...
module.exports.getAllUsers = async (req, res, next) => {
  try{
    const foundUsers = await bd.Users.findAll()
    res.send(foundUsers)
  }catch(e){
...
module.exports.updatePass = async (req, res, next) => {
  try {
    ...
    const {id} = foundUser;
    const pass = req.body.hashPass
    await userQueries.updateUser({ password: pass }, id);
...
module.exports.restore = async (req, res, next) => {
  try {
    const foundUser = awaitrouter.post(
      '/changeMark',
      checkToken.checkToken,
      basicMiddlewares.onlyForCustomer,
      userController.changeMark,
    ); userQueries.findUser({ email: req.body.email });
    const hashPassw = req.hashPass
    console.log('User Email: ' + foundUser.email + ' New Password: ' + req.body.password + ' hashPass ' +req.hashPass )
    sendMail.passRestore(foundUser.email, hashPassw);
  } catch (err) {
    next(err);
  }
};
